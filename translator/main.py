...
def handle_save_path(_save_path, translations, p, spinner):
...
def is_asking_for_help(_from):
    return "".join(_from).lower() in fetch_help

def is_asking_for_version(version, _from):
    return version or "".join(_from).lower() in fetch_version

def is_asking_for_languages(language_list, _from):
    return language_list or "".join(_from).lower() in fetch_languages

def is_asking_to_translate_po_files_from_directory(_po_mode, _directory):
    return _po_mode and _directory and Path(_directory).exists()

def is_asking_to_translate_single_po_file(_sentences, _po_mode):
    return _po_mode and _sentences and len(_sentences) == 1 and _sentences[0].endswith('.po')

def is_asking_to_translate_text_files_from_directory(_sentences, _directory, _po_mode):
    return not _po_mode and not _sentences and _directory and Path(_directory).exists()

def is_default_model(model_id):
    return model_id == default_translator_model

def is_interactive_mode(is_interactive, _from, _to, sentences, directory, po_mode, _version, _langues_list):
    return is_interactive and not (is_asking_for_help(_from) or is_asking_for_version(_version, _from) or is_asking_for_languages(_langues_list, _from)) and (not _to and not sentences and not directory) and not po_mode

def display_help(parser):
    parser.print_help()

def display_version(is_interactive, spinner, max_length, model_id, pipeline, batch_size, _to=None):
    print_version(__version__, _to=_to, is_interactive=is_interactive, spinner=spinner, logger=logger, max_length=max_length, model_id=model_id, pipeline=pipeline, batch_size=batch_size)

def display_languages(spinner, model_id):
...
def handle_conflicting_flags(args):
    if args.interactive and (args.version or args.language_list):
        args.interactive = False
        # _log("Warning: --interactive flag is ignored when --version or --language_list is used.", logger, None, 'warning')
    
    # Validate conflicting flags
    if args.po and args.save:
        _log("Error: --po and --save flags cannot be used together. PO files are translated in-place.", logger, None, 'error')
        sys.exit(1)

    if args.po and not args.directory:
        _log("Error: --po flag requires --directory to be specified.", logger, None, 'error')
        sys.exit(1)

    
    return args

def handle_po_mode(args, _from, _to, spinner):
    if args.po:
...
def handle_interactive_mode(args, _from, _to, _sentences, _directory, is_interactive, spinner):
...
def handle_missing_flags(spinner, args, _from, _to, _sentences, _directory, _save_path, _po_mode):
...
def handle_invalid_flags(spinner, args, _from, _to, _sentences, _directory, _save_path, _po_mode):
...
def handle_po_directory_translation(
    translator,
    _directory,
    _to,
    _force,
    ...
    if is_asking_to_translate_po_files_from_directory(_po_mode, _directory):
...
def handle_single_po_file_translation(
    translator,
    _sentences,
    _to,
    _force,
    ...
    if is_asking_to_translate_single_po_file(_sentences, _po_mode):
...
def handle_text_files_translation_from_directory(
    args,
    translator,
    _from,
    _to,
...
def handle_text_files_translation(
    args,
    translator,
    _sentences,
    _po_mode,
    ...
    if is_asking_to_translate_text_files_from_directory(_sentences, _directory, _po_mode):
        handle_text_files_translation_from_directory(
            args,
            translator,
            _from,
            _to,
        ...
        handle_sentences_translation(
            translator,
            _sentences,
            _po_mode
        )

def handle_sentences_translation(
    translator,
    _sentences,
    translations
):
...
def start_translation(
    ...
    handle_po_directory_translation(
        translator,
        _directory,
        _to,
        _force,
    ...
    handle_single_po_file_translation(
        translator,
        _sentences,
        _from,
        _to,
    ...
    translations, p = handle_text_files_translation(
        args,
        translator,
        _sentences,
        _po_mode,
...
def handle_flags(flags):
    args, parser = flags["args"], flags["parser"]

    if args.debug:
        logger.setLevel(logging.DEBUG)
        logging.getLogger('translator.translate').setLevel(logging.DEBUG)

    args = handle_conflicting_flags(args)

    is_interactive = args.interactive
    if is_interactive:
        spinner = Halo(spinner=dots_design)
    else:
        spinner = None
    ...
    if is_asking_for_help(args._from):
        display_help(parser)
        sys.exit(0)

    if is_asking_for_version(args.version, args._from):
        display_version(is_interactive, spinner, args.max_length, args.model_id, args.pipeline, args.batch_size, _to=_to)
        sys.exit(0)

    if is_asking_for_languages(args.language_list, args._from):
        display_languages(is_interactive, spinner, args.model_id)
        sys.exit(0)

    _from, _to = handle_po_mode(args, _from, _to, spinner)

    nepoch, nproc, batch_size = args.nepoch, args.nproc, args.batch_size

    if is_interactive_mode(is_interactive, _from, _to, _sentences, _directory, _po_mode):
        (
            _from,
            _to,
            _sentences,
            ...
            batch_size,
            nproc
        ) = handle_interactive_mode(args, _from, _to, _sentences, _directory, is_interactive, spinner)

    _to, _sentences = handle_missing_flags(spinner, args, _from, _to, _sentences, _directory, _save_path, _po_mode)
    _from, _to = handle_invalid_flags(spinner, args, _from, _to, _sentences, _directory, _save_path, _po_mode)

    translations, p = start_translation(
        is_interactive,
        spinner, 
        args, 
        _from, 
    ...
    if _save_path:
        handle_save_path(_save_path, translations, p, spinner, logger)
